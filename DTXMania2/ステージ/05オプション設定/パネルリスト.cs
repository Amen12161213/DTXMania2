        ...
        public パネルリスト()
        {
            ...
            this.パネルツリーのルートノード.子パネルリスト.Add(
                ...
                new パネル_文字列リスト(
                    ...
                    選択肢初期値リスト:
                        new[] { "BASIC", "ADVANCED", "EXPERT" },

                    初期選択肢番号:
                        (int)userConfig.演奏モード,

                    値の変更処理:
                        (panel) => {
                            userConfig.演奏モード = (PlayMode)((パネル_文字列リスト)panel).現在選択されている選択肢の番号;
            ...
            //----------------
            {
                ...
                this.パネルツリーのルートノード.子パネルリスト.Add(
                    ...
                    new パネル_文字列リスト(
                        ...
                        初期選択肢番号:
                            (選択肢リスト.Contains(userConfig.レーン配置)) ? 選択肢リスト.IndexOf(userConfig.レーン配置) : 0,

                        値の変更処理:
                            (panel) => {
                                userConfig.レーン配置 = 選択肢リスト[((パネル_文字列リスト)panel).現在選択されている選択肢の番号];
                            }
                    ));
            }
            //----------------
